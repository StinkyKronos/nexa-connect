...
export default function Home() {
	...
	const handleSharingChange = (value) => {
		setSharing((prev) => (prev.includes(value) ? prev.filter((s) => s !== value) : [...prev, value]));
	};
				...
				<div className="flex-[0.3]">
					<form action="" className="flex flex-col items-center justify-center gap-5 bg-[#D9D9D9] py-10 px-10 rounded-xl">
						<div className="bg-white w-full px-5 py-2 rounded-xl">
							...
							/>
							<p className="text-[#848484] poppins-medium">Dist. from College: {(distance / 1000).toFixed(1)} km</p>
						</div>
						...
						<div className="bg-white w-full px-5 py-2 rounded-xl">
							...
							<div className="flex gap-5 mt-2">
								<div className="w-full">
									<input className="border-[#FC813E] bg-[#FFF3ED] border-2 poppins-medium w-full text-black" type="text" min={0} max={50000} defaultValue={0} onChange={(e) => setMinBudget(e.target.value)} />
									<p className="text-[#848484] poppins-medium w-fit">min.</p>
								</div>
								<div className="w-full">
									<input className="border-[#FC813E] bg-[#FFF3ED] border-2 poppins-medium w-full text-black" type="text" min={1} max={50000} defaultValue={50000} onChange={(e) => setMaxBudget(e.target.value)} />
						...
						<div className="bg-white w-full px-5 py-5 rounded-xl flex items-center justify-center flex-col gap-3">
							...
							<div className="grid grid-cols-2 w-full h-fit gap-5">
								<CheckboxSearch title="Single" value={"Single"} handler={handleSharingChange} />
								<CheckboxSearch title="Double" value={"Double"} handler={handleSharingChange} />
								<CheckboxSearch title="Triple" value={"Triple"} handler={handleSharingChange} />
								<CheckboxSearch title="Four" value={"Four"} handler={handleSharingChange} />
							</div>
						</div>
						...
						<div className="bg-white w-full px-5 py-5 rounded-xl flex items-center justify-center flex-col gap-3">
							...
							<div className="grid grid-cols-2 w-full h-fit gap-5">
								<CheckboxSearch title="Male" value={"Male"} handler={handleGenderChange} />
								<CheckboxSearch title="Female" value={"Female"} handler={handleGenderChange} />
							</div>
						</div>
					</form>
				</div>
				<div className="flex-[0.7]">
					<div className="w-full h-full">
						...
						<div className="h-[60vh] overflow-auto scrollbar-hidden my-5">
							{filterProperties ? filterProperties.map((pro) => <PropertyCard key={pro.id} name={pro.property_name} rent={pro.rent} occupancy={pro.occupancy} />) : <h3>No properties found</h3>}
						</div>
					</div>
				</div>
